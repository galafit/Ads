...
public class EdfHeader extends DefaultEdfConfig{
    ...
    private static final String ERR_MSG_START = "HeaderRecord error! ";
    ...
    private void readBuffer(Reader reader, char[] buffer) throws IOException {
        String errMsg = MessageFormat.format("{0}HeaderRecord does not contain the required information.", ERR_MSG_START);
        HeaderFormatException headerNotFullException = new HeaderFormatException(ExceptionType.HEADER_NOT_FULL, errMsg);
     ...
     */
    EdfHeader(File file) throws EdfRuntimeException, EdfHeaderRuntimeException {
        try {
            ...
            if ((Character.getNumericValue(firstChar) & 0xFF) == 255) { // BDF
                ...
                if(version.equals(expectedVersion)) {
                    ...
                    String errMsg = MessageFormat.format("{0}HeaderRecord has unknown version: \"{1}\". Expected: \"{2}\"", ERR_MSG_START, version, expectedVersion);
...
/**************************** PHYSICAL AND DIGITAL RANGES ***********************/
            ...
            for (int signalNumber = 0; signalNumber < numberOfSignals; signalNumber++) {
                ...
                if(digMinList[signalNumber] < dataFormat.getDigitalMin() || digMinList[signalNumber] >= dataFormat.getDigitalMax()) {
                    String expected = dataFormat.getDigitalMin()+" <= digital min < "+ dataFormat.getDigitalMax();
                    ...
                    ex.setRange(dataFormat.getDigitalMin(), dataFormat.getDigitalMax());
                ...
                if(digMaxList[signalNumber] <= dataFormat.getDigitalMin() || digMaxList[signalNumber] > dataFormat.getDigitalMax()) {
                    String expected = dataFormat.getDigitalMin()+" < digital max <= "+ dataFormat.getDigitalMax();
...
/*******************************************************************************/
    ...
    public byte[] createFileHeader() throws EdfHeaderRuntimeException{
        ...
        for (int signalNumber = 0; signalNumber < getSignalsCount(); signalNumber++) {
        ...
        headerBuilder.append(adjustLength(dataFormat.getVersion(), VERSION_LENGTH - 1));  // -1 because first non ascii byte (or "0" for edf) we will add later
        ...
        headerBuilder.append(adjustLength(dataFormat.getFirstReserved(), FIRST_RESERVED_LENGTH));
        ...
        headerBuilder.append(adjustLength(Integer.toString(getSignalsCount()), NUMBER_OF_SIGNALS_LENGTH));
        ...
        byteBuffer.put(dataFormat.getFirstByte());
    ...
    public static void main(String[] args) {
        ...
        HeaderConfig headerConfigEdf = new HeaderConfig(numberOfSignals, com.biorecorder.edflib.FileType.EDF_16BIT);
        HeaderConfig headerConfigBdf = new HeaderConfig(numberOfSignals, com.biorecorder.edflib.FileType.BDF_24BIT);
